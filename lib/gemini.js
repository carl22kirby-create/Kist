...
// gemini-1.5-flash was retired at some point after this was first built —
// confirmed live, via a direct curl-equivalent test against Google's API,
// that gemini-3.6-flash is the current stable model that actually works.
// Overridable via GEMINI_MODEL if Google moves on again before this gets
// revisited — check https://ai.google.dev/gemini-api/docs/models for the
// current recommended stable model if this ever starts failing again.
...
export async function askGemini(systemInstruction, userQuestion, responseSchema = RESPONSE_SCHEMA) {
  if (!GEMINI_API_KEY) {
