...
export default function Dashboard({ data, setPage, setSelectedClient, setCalendarAnchor }) {
  const [detail, setDetail] = useState(null);
  const [showEdit, setShowEdit] = useState(false);
  const [widgets, setWidgets] = useState(
    () => JSON.parse(localStorage.getItem("kist_widgets_v220") || "null") || {
      metrics: true, diary: true, health: true, dna: true, reports: true, actions: true, ai: true
    }
  );

  function saveWidgets(next) {
    setWidgets(next);
    localStorage.setItem("kist_widgets_v220", JSON.stringify(next));
  }
